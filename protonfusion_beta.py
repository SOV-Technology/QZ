...
try:
    ...
    # Validate we have at least Hydrogen
    if "1" not in periodic_table:
        raise ValueError("JSON must contain at least Hydrogen (key '1')")
...
def draw_element_visual(screen, element, center_x, center_y, time):
    ...
    if game_state.show_orbitals:
        ...
        for shell, max_electrons in enumerate(shell_config, 1):
            ...
            orbital_color = (red, 50, blue)
            # Create surface for orbital with alpha
            s = pygame.Surface((radius*2, radius*2), pygame.SRCALPHA)
            pygame.draw.circle(s, (*orbital_color, alpha), (radius, radius), radius, 1)
    ...
    for i in range(proton_count + neutron_count):
        ...
        if i < proton_count:
            pygame.draw.circle(screen, (255, 100, 100), (int(x), int(y)), int(nucleon_radius))
        else:
            pygame.draw.circle(screen, (200, 200, 255), (int(x), int(y)), int(nucleon_radius))
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_SPACE or event.key == pygame.K_RIGHT:
                ...
                next_num = (current_num % len(periodic_table)) + 1
                while str(next_num) not in periodic_table and next_num <= len(periodic_table):
                    next_num = (next_num % len(periodic_table)) + 1
                game_state.selected_element_key = str(next_num)
                ...
                # Previous element
                current_num = int(game_state.selected_element_key)
                prev_num = (current_num - 2) % len(periodic_table) + 1
                while str(prev_num) not in periodic_table and prev_num >= 1:
                    prev_num = (prev_num - 2) % len(periodic_table) + 1
                game_state.selected_element_key = str(prev_num)
                game_state.current_element = periodic_table[game_state.selected_element_key]
                play_element_sound(game_state.current_element)
